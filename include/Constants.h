...
constexpr vec3 WORLDSPACE_UP = vec3(0.0f, 1.0f, 0.0f);
...
inline float randomFloat() { return std::rand() / (static_cast<float>(RAND_MAX) + 1.0f); }
inline float randomFloat(float min, float max) { return min + (max - min) * randomFloat(); }

inline vec3 random_vec3() { return vec3(randomFloat(), randomFloat(), randomFloat()); }
inline vec3 random_vec3(float min, float max) { return vec3(randomFloat(min, max), randomFloat(min, max), randomFloat(min, max)); }
...
inline vec3 randomInUnitDisk() {
    ...
    while (iter++ < 1000) {
        ...
        if (p.x * p.x + p.y * p.y < 1.0f) return p;
    }
    throw std::runtime_error("Too many iterations in randomInUnitDisk!");
}